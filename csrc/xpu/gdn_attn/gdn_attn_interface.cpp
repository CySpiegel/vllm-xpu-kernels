...
void gated_delta_rule(
  ...
  if (non_spec_token > 0) {
    ...
    if (num_prefills > 0) {
      ...
      // within every sequence. That holds for the engine's current layouts
      // but is an unchecked contract; arbitrary index sets (and any future
      // layout change) corrupt neighboring rows or overrun the tensor. Make
      // the assumption true by construction: with token_indx present, write
      // into a compact staging buffer with identity indexing, then scatter
      // per-row.
      if (non_spec_token_indx.has_value()) {
        ...
        chunk_gated_delta_rule_xe2(
            queue, o_compact, q, k, v, b, a, A_log, dt_bias, ssm_state,
            *non_spec_query_start_loc, *non_spec_state_indices_tensor,
            has_initial_state, num_prefills, num_decodes,
            /*token_indx=*/nullptr);
        core_attn_out_active.index_copy_(
            0, non_spec_token_indx->to(torch::kLong), o_compact);
      } else {
        chunk_gated_delta_rule_xe2(
            queue, core_attn_out_active, q, k, v, b, a, A_log, dt_bias,
            ssm_state, *non_spec_query_start_loc,
            *non_spec_state_indices_tensor, has_initial_state, num_prefills,
            num_decodes, /*token_indx=*/nullptr);
      }
    } else {
      gdn::gated_delta_rule(
          queue,
